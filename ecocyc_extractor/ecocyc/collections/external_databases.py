...
class ExternalDatabases(object):
    ...
    def get_ids(registered_ids=False):
        if registered_ids:
            ...
            external_db_ids = ExternalDatabases.pt_connection.get_class_all_instances(EC.EXTERNAL_DB_CLASS)
        return external_db_ids

    @property
    def objects(self):
        external_db_objects = ExternalDatabases.pt_connection.get_frame_objects(self.ids)
        for external_db in external_db_objects:
            external_db = ExternalDatabases.set_external_db(external_db)
            logging.info('Working on external_db: {}'.format(external_db["id"]))
