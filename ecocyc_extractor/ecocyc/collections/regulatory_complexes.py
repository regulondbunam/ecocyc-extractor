...
class RegulatoryComplexes(object):
        ...
        if ids is None:
            ...
            if include_inactive is True:
                for tf_id in RegulatoryComplexes.transcription_factor_ids:
                    inactive_conformation_ids = TranscriptionFactor.get_tf_inactive_conformations(
                        tf_id)
                    regulatory_complex_ids.extend(inactive_conformation_ids)
        else:
            regulatory_complex_ids = ids
        regulatory_complex_ids = utils.get_unique_elements(
            regulatory_complex_ids)
        return regulatory_complex_ids

    @property
    def objects(self):
        regulatory_complex_objects = RegulatoryComplexes.pt_connection.get_frame_objects(
            self.ids)
        for regulatory_complex in regulatory_complex_objects:
            regulatory_complex = RegulatoryComplexes.set_regulatory_complex(
                regulatory_complex)
            logging.info('Working on regulatory complex: {}'.format(
                regulatory_complex["id"]))
    ...
    def get_regulator_ids(regulatory_interaction_ids):
        ...
        for ri_id in regulatory_interaction_ids:
            regulator_id = RegulatoryComplexes.pt_connection.get_slot_value(
