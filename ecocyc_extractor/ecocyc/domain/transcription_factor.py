...
class TranscriptionFactor(Base):
    ...
    def active_conformations(self, active_conformations):
        if active_conformations is None:
            ...
            for conformation_id in TranscriptionFactor.get_tf_active_conformations(
                self.id
            ):
    ...
    def inactive_conformations(self, inactive_conformations):
        if inactive_conformations is None:
            ...
            for conformation_id in TranscriptionFactor.get_tf_inactive_conformations(
                self.id
            ):
    ...
    def get_products_ids(self):
        ...
        if (
            EC.POLYPEPTIDE_CLASS in parent_classes
            or EC.PSEUDO_PRODUCT_CLASS in parent_classes
        ):
    ...
    def get_tf_active_conformations(transcription_factor_id):
        ...
        for container_of_id in containers_of_tf:
            ...
            if regulates and any(
                protein_class in EC.PROTEIN_CLASSES for protein_class in parent_classes
            ):
